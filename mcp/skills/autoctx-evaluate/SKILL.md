...
3. **Config Generation (Core Execution):**
   - Use the `generate_evalbench_configs` MCP tool.
   - Provide the selected `experiment_name`, `dataset_path`, `context_set_id`, and the fully extracted `toolbox_db_info` block as a JSON string mapping.
